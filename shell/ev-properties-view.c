	      ...
	      gint             *row)
{
	...
	if (!properties->labels[property]) {
		gtk_widget_set_hexpand(label, TRUE); gtk_grid_attach (GTK_GRID(table), label, 1, *row, 1, 1);
...
ev_properties_view_set_info (EvPropertiesView *properties, const EvDocumentInfo *info)
{
	...
	if (info->fields_mask & EV_DOCUMENT_INFO_TITLE) {
		set_property (properties, GTK_GRID (table), TITLE_PROPERTY, info->title, &row);
	}
	set_property (properties, GTK_GRID (table), URI_PROPERTY, properties->uri, &row);
	if (info->fields_mask & EV_DOCUMENT_INFO_SUBJECT) {
		set_property (properties, GTK_GRID (table), SUBJECT_PROPERTY, info->subject, &row);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_AUTHOR) {
		set_property (properties, GTK_GRID (table), AUTHOR_PROPERTY, info->author, &row);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_KEYWORDS) {
		set_property (properties, GTK_GRID (table), KEYWORDS_PROPERTY, info->keywords, &row);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_PRODUCER) {
		set_property (properties, GTK_GRID (table), PRODUCER_PROPERTY, info->producer, &row);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_CREATOR) {
		set_property (properties, GTK_GRID (table), CREATOR_PROPERTY, info->creator, &row);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_CREATION_DATE) {
		text = g_date_time_format (info->creation_date, "%c");
		set_property (properties, GTK_GRID (table), CREATION_DATE_PROPERTY, text, &row);
		g_free (text);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_MOD_DATE) {
		text = g_date_time_format (info->modified_date, "%c");
		set_property (properties, GTK_GRID (table), MOD_DATE_PROPERTY, text, &row);
		g_free (text);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_FORMAT) {
		set_property (properties, GTK_GRID (table), FORMAT_PROPERTY, info->format, &row);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_N_PAGES) {
		text = g_strdup_printf ("%d", info->n_pages);
		set_property (properties, GTK_GRID (table), N_PAGES_PROPERTY, text, &row);
		g_free (text);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_LINEARIZED)
	{
		...
		{
			set_property (properties, GTK_GRID (table), LINEARIZED_PROPERTY, _("Yes"), &row);
		}
		else if (info->linearized == (char *)0)
		{
			set_property (properties, GTK_GRID (table), LINEARIZED_PROPERTY, _("No"), &row);
		}
		else
		{
			set_property (properties, GTK_GRID (table), LINEARIZED_PROPERTY, info->linearized, &row);
		}
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_SECURITY) {
		set_property (properties, GTK_GRID (table), SECURITY_PROPERTY, info->security, &row);
	}
	if (info->fields_mask & EV_DOCUMENT_INFO_PAPER_SIZE) {
		text = ev_regular_paper_size (info);
		set_property (properties, GTK_GRID (table), PAPER_SIZE_PROPERTY, text, &row);
		g_free (text);
	}
}
...
ev_properties_view_init (EvPropertiesView *properties)
{
	...
	properties->table = gtk_grid_new ();
	gtk_grid_set_column_spacing (GTK_GRID (properties->table), 12);
	gtk_grid_set_row_spacing (GTK_GRID (properties->table), 6);
	gtk_widget_set_margin_start (properties->table, 12);
	gtk_widget_set_margin_end (properties->table, 12);
	gtk_widget_set_margin_top (properties->table, 12);
	gtk_widget_set_margin_bottom (properties->table, 12);
	
	gtk_widget_set_vexpand (properties->table, TRUE);
	gtk_widget_set_hexpand (properties->table, TRUE);
	gtk_box_append (GTK_BOX (properties), properties->table);
