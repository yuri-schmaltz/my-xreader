...
ev_annotation_properties_dialog_init (EvAnnotationPropertiesDialog *annot_dialog)
{
	...
	button = gtk_dialog_add_button (dialog, (_("_Apply")), GTK_RESPONSE_APPLY);
	...
	gtk_widget_set_margin_start(grid, 12); gtk_widget_set_margin_end(grid, 12); gtk_widget_set_margin_top(grid, 12); gtk_widget_set_margin_bottom(grid, 12);
	...
	gtk_label_set_markup (GTK_LABEL (label), markup);
	g_free (markup);
	gtk_widget_set_halign(label, GTK_ALIGN_END); gtk_widget_set_hexpand(label, TRUE); gtk_box_append (GTK_BOX (hbox), label);
	gtk_widget_show (label);
